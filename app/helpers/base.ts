...
var Base = {
  ...
  doRequest: (url: string, next: Function, parse: boolean = true) => {
    ...
    xhr.onload = () => {
      if (xhr.readyState === 4) {
        if (xhr.status === 200) {
          parse ? next(JSON.parse(xhr.responseText)) : next(xhr.responseText);
        } else {
          next({});
        }
      } else {
  ...
  pointInBounds: (point: Array<number>, extent: Array<number>): boolean => {
    if (point.length === 2 && extent.length === 2) {
      const x = point[1];
      const y = point[0];
      return (
        extent[0][1] < x &&
